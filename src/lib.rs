...
use juniper::{Arguments, ExecutionResult, Executor, GraphQLType, GraphQLValue, Registry, ScalarValue, Value};
...
impl<S> GraphQLType<S> for JsonObject
    where
        S: ScalarValue,
{
            ...
            S: 'r,
    {
        ...
        registry
            .build_object_type::<JsonObject>(info, &fields)
            .into_meta()
    }
}

fn build_field<'r, 't, S, T>(info: &SchemaTypeInfo, registry: &mut Registry<'r, S>, field_name: &str, type_ref: Type<'t, T>, nullable: bool) -> Field<'r, S>
    ...
    match type_ref {
        ...
        Type::NamedType(type_name) => {
            match type_name.as_ref() {
                ...
                _ => {
                    let field_node_type_info = &SchemaTypeInfo {
                        schema: info.schema.clone(),
                        name: type_name.clone().as_ref().to_string(),
                    };
                    if nullable {
                        registry.field::<Option<JsonObject>>(field_name, field_node_type_info)
                    } else {
                        registry.field::<JsonObject>(field_name, field_node_type_info)
                    }
                }
            }
        }
